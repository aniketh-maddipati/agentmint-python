...
if __name__ == "__main__":
    ...
    notary.notarise(
        action="ehr:read:patient", agent="clinical-ops", plan=plan,
    ...
    notary.notarise(
        action="billing:charge:payment", agent="clinical-ops", plan=plan,
    ...
    if s3.blocked:
        notary.notarise(
            action="shield:blocked:notify:send:message",
            agent="clinical-ops",
            plan=plan,
            evidence={
                "original_action": "notify:send:message",
                "recipient": "maria.chen@email.com",
                "shield": s3.summary(),
                "blocked": True,
            },
        )
        print(f"    BLOCKED — tool call never executed")
    else:
        r3 = _do_notify("maria.chen@email.com", bad_msg)
        notary.notarise(
            action="notify:send:message", agent="clinical-ops", plan=plan,
            evidence={"channel": "email", "recipient": "maria.chen@email.com"},
        )
    ...
    notary.notarise(
        action="audit:query:log", agent="clinical-ops", plan=plan,
    ...
    print("=" * 60)
    print(f"  {len(receipts)} tool calls → signed, chained, timestamped, verified")
