...
- Checks the requested operation against the **scope**; if out of scope, it is blocked or marked as a violation.
- Enforces **checkpoints** for high‑risk actions, so those cannot execute without additional approval.
- Adds **replay protection**, so a captured plan cannot be reused indefinitely.

Per‑agent, per‑action authorization — not "this API key can do everything forever."
...
IAM decisions are no longer just runtime checks — they're **persisted as tamper‑evident evidence** you can show auditors, insurers, or platforms like AIUC‑1 / IBM Risk Atlas.
...
| Receipt Field          | AIUC‑1 Control(s) | What It Proves                                                                            |
| ---------------------- | ----------------- | ----------------------------------------------------------------------------------------- |
...
- Running against production APIs: ElevenLabs, Claude, AWS S3
- 80 receipts generated and verified in a single batch run
...
MVP generating real receipts against production APIs.
Schema actively evolving with AIUC-1 / ISO 42001 / EU AI Act alignment.
...
[Aniketh Maddipati](https://linkedin.com/in/anikethmaddipati) · NYC · anikethcov@gmail.com